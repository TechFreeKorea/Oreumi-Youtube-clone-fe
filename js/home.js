...
async function createVideoItem(videoList) {
  ...
  for (let i = 0; i < videoList.length; i++) {
    ...
    let channelInfo = await getChannelInfo(videoList[i].channel_id);

    let channelURL = `./channel.html?channelId=${videoList[i].channel_id}"`;
          ...
          <div class="feed__item__info">
            ...
            <div class="feed__text__box">
              <h3 class="feed__item__info__title">
                  <a href='${videoURL}'> ${videoInfo.title}</a>
              </h3>
              <a href="${channelURL}">${channelInfo.channel_name}</a>
